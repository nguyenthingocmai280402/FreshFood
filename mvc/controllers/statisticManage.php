    ...
    public function revenue()
{
    if (isset($_GET['from']) && isset($_GET['to'])) {
        $statistic = $this->model("statisticModel");
        $statisticType = isset($_GET['statistic_type']) ? $_GET['statistic_type'] : 'monthly';

        if ($statisticType == 'monthly') {
            $result = $statistic->getMonthlyRevenue($_GET['from'], $_GET['to']);
        } elseif ($statisticType == 'quarterly') {
            $result = $statistic->getQuarterlyRevenue($_GET['from'], $_GET['to']);
        } elseif ($statisticType == 'yearly') {
            $result = $statistic->getYearlyRevenue($_GET['from'], $_GET['to']);
        } else {
            // Xử lý sai lựa chọn thống kê ở đây nếu cần
        }
